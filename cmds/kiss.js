...
module.exports = {
    name: 'kiss',
    description: 'Sends an embed with an anime kiss image for the mentioned or replied-to user.',
    async execute(client, message, args, settings) {
        console.log(`[DEBUG] Starting kiss command in channel ${message.channel.id} at ${new Date().toISOString()}`);
        ...
        try {
            ...
            console.log(`[DEBUG] Fetched kiss image URL: ${imageUrl}`);
        } catch (error) {
        ...
        const embed = {
            title: "mwah!! luv u :3",
            description: `> <@${message.author.id}> gives <@${userId}> a sweet kiss!\nFeel the love!`,
