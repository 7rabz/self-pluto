...
module.exports = {
    name: 'cuddle',
    description: 'Sends an embed with an anime cuddle image for the mentioned or replied-to user.',
    async execute(client, message, args, settings) {
        console.log(`[DEBUG] Starting cuddle command in channel ${message.channel.id} at ${new Date().toISOString()}`);
        ...
        try {
            ...
            console.log(`[DEBUG] Fetched cuddle image URL: ${imageUrl}`);
        } catch (error) {
            console.error(`[ERROR] Failed to fetch cuddle image: ${error.message}`);
            return;
        }

        const embed = {
            title: "mm so warm :3",
            description: `> <@${message.author.id}> cuddles <@${userId}> warmly!\nEnjoy the cozy moment!`,
