...
module.exports = {
    name: 'flipoff',
    description: 'Sends an embed with an anime flip-off image for the mentioned or replied-to user.',
    async execute(client, message, args, settings) {
        console.log(`[DEBUG] Starting flipoff command in channel ${message.channel.id} at ${new Date().toISOString()}`);
        ...
        let imageUrl;
        try {
            const response = await axios.get('https://nekos.best/api/v2/baka'); // Using baka as a fallback for cheeky gesture
            imageUrl = response.data.results[0].url;
            console.log(`[DEBUG] Fetched flipoff image URL: ${imageUrl}`);
        } catch (error) {
            console.error(`[ERROR] Failed to fetch flipoff image: ${error.message}`);
            return;
        }

        const embed = {
            title: "grrr fwick you!! >:3",
            description: `> <@${message.author.id}> flips off <@${userId}> with attitude!\nTake that!`,
