    ...
    async execute(client, message, args, settings) {
        ...
        try {
            ...
            if (!match) {
                embed = {
                    title: "Error",
                    description: `Please provide both a poll question and options in double quotes.\nExample: ${settings.commandPrefix}poll "Your question" "Option1,Option2,Option3"`,
                    color: 0xff0000,
                ...
                if (pollOptions.length < 2 || pollOptions.length > 9) {
                    ...
                    const pollDescription = pollOptions.map((option, index) => `${numberEmojis[index]}: ${option}`).join("\n");

                    embed = {
                        title: `:question: | ${pollQuestion} | :question:`,
